const { Report, User, Subject, Chapter } = require('../models');

class ReportController {
  static async show(req, res, next) {
    try {
      const { grade } = req.query;
      const { dataValues: checkToken } = req.authentication;
      let where = {
        grade: +grade
      };
      ...
      if (findUser.role === 'STUDENT') {
        if (+grade !== findUser.grade) {
          next({ detailedCode: 'Unauthorized' });
        }
      }
      if (!grade) {
        delete where.grade;
      }
      const students = await Report.findAll({
        where,
      ...
      let findUserAll = await User.findAll({
        where
      });
      let output = [];
      ...
      for (let j of findUserAll) {
        ...
        for (let i of students) {
          if (user.name === i.dataValues.User.name) {
            let data = {
              subject: i.dataValues.Subject.subjectName,
              chapter: i.dataValues.Chapter.chapterName,
              score: i.dataValues.score
            };
            switch (i.dataValues.Subject.subjectName) {
              case 'Math':
      ...
      if (findUser.role === 'STUDENT') {
        const returning = output.find((x) => x.name === findUser.name);
        res.status(200).json(returning);
      }
      res.status(200).json(output);
    } catch (error) {
      next({ detailedCode: 'Internal Server Error' });
    }
  }
}
